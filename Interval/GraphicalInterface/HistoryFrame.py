...
class HistoryTranslation(HistoryNotepage):
    def BuildPage(self):
        ...
        self.refreshButton = myButton(controls, row=1, col=1, command=self.refreshHistory, rowspan=1, colspan=1,
                                      sticky='W', text='Refresh', bg='white', fg='black', font=('Arial', 12),
                                      relief='raised')
        self.clearButton = myButton(controls, row=1, col=2, command=self.clearHistory, rowspan=1, colspan=1,
        ...
        self.scrollBar = myScrollBar(txtFrame, row=1, col=0, stick='ew',orient='horizontal',columnspan=100)
    ...
    def selectItem(self):
        ...
        selectedTranslation = re.sub(r'[0-9]*\. ','',selectedTranslation)  # To remove the index inserted in the history box.
